...
PDF rendering uses the pdf.js worker, which is **embedded directly inside `main.js`** — there is no separate `pdf.worker.js` file and no network request, so PDFs work fully offline. Only if that embedded worker fails to initialize does the plugin fall back to fetching the worker from a CDN (`cdnjs.cloudflare.com`).
...
> Each GitHub release must include **`manifest.json`**, **`main.js`** and **`styles.css`**. The pdf.js worker is embedded inside `main.js`, so there is no separate `pdf.worker.js` to attach.

## 🛠️ Manual install (without BRAT)

1. Download `main.js`, `styles.css` and `manifest.json` from a release.
...
```bash
npm install
npm run build      # produces main.js (with the pdf.js worker embedded)
npm run dev        # same, in watch mode with an inline source map
```

`main.js` in the repository root is the build output that Obsidian loads. The
pdf.js worker is **embedded into `main.js`** at build time — read from
`pdfjs-dist`, patched in memory for the sentence-period fix (see
`patch-pdfjs.cjs`) — so PDFs keep working offline with no separate worker file
to ship. `main.js` is regenerated by `npm run build` — edit `src/main.js`,
never the bundle.
...
> ⚠️ Чтобы BRAT увидел версию, в каждом GitHub-релизе должны быть приложены файлы **`manifest.json`**, **`main.js`** и **`styles.css`**. Воркер pdf.js встроен внутрь `main.js`, поэтому отдельный `pdf.worker.js` прикладывать не нужно.
